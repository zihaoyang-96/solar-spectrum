; NAME: int_h
; PURPOSE: to calculate the change of line intensity along the radial distance, using the density model of Gibson et al.(1999)
;         and assuming the temperature below 1.5R_sun is logt=6.2, and after that using the temperature model mentioned in Del Zanna et al.(2018)
;         the EM is assumed at logem=27.8 when the electron density is about logn=9.
...
;n_e=a*exp(b/r)*r^(-2)*(1+c/r+d/(r^2)+e/(r^3))
logn=alog10(n_e)
...
t[0:5]=10^6.2
;t[0:5]=10^(findgen(6)*0.01+6.14)
;t=1e6/(0.35*r^(1.1)+1.9*r^(-6.6))
...
for i=0,5 do begin
  ...
  ;make_chianti_spec,tran,lambda,str,bin_size=0.01,instr_fwhm=0.1,wrange=[wmin,wmax],abund_name=concat_dir(concat_dir(!xuvtop,'abundance'),'sun_photospheric_2009_asplund.abund')
